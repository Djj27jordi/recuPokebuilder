...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // ACTIVA EL FILTRE SI VE DE LA URL
  const urlParams = new URLSearchParams(window.location.search);
  const tipus = urlParams.get('tipus');
  if (tipus) {
    // Busca el filtre pel seu data-nomF
    let filtre = document.querySelector(`.filtro[data-nomF="${tipus}"]`);
    if (filtre) {
      filtre.setAttribute("data-filtre", "true");
      filtre.classList.add("filtroActiu");
      filtre1 = tipus;

      // Crida AJAX per mostrar només aquest tipus
      let web = new XMLHttpRequest();
      ...
      web.send("filtre1=" + tipus + "&filtre2=");
    }
  }
